...
class IllustPage extends StatefulWidget {
  ...
  const IllustPage(this.illust, {Key? key, this.parentModel}) : super(key: key);
...
class _IllustPageState extends State<IllustPage> {
  Widget _buildBookmarkButton(IllustModel model) {
  ...
  List<Widget> _buildRelated(IllustModel model) {
    final list = <Widget>[];
  ...
  Widget build(BuildContext context) {
    return ProviderWidget(
      model: IllustModel(widget.illust),
      ...
      onModelReady: (IllustModel model) => model.loadFirstData(),
    );
  }
}
